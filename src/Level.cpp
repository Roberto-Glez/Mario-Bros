...
Level::Level(Physics &physics, float width, float height)
  ...
  // Config Setup
  m_groundY =
      height - 32.0f; // Adjusted to be 2 blocks high (16*2=32) or just 1?
  ...
  m_groundVertices.setPrimitiveType(
      sf::PrimitiveType::Triangles); // SFML 3: Quads removed
  m_groundVertices.resize(numTilesX * numTilesY *
                          6); // 6 vertices per tile (2 triangles)
  ...
  // Or based on the attached image (brown block with corner details), it looks
  // like the standard breakable brick or the floor block. Let's guess (0, 0)
  // for now. Actually, looking at common spritesheets: (0,0) is often a brick
  // or question block. The ground block is often distinct. I will start with
  // (0,0) and adjust. Width 16, Height 16.
  ...
  b2Polygon wallBox =
      b2MakeBox(10.0f / Physics::SCALE,
                (height / 2.0f) / Physics::SCALE); // Altura completa
  ...
  if (m_stompCooldown <= 0.0f) {
    for (auto &enemy : m_enemies) {
      ...
      if (pBounds.findIntersection(enemyBounds)) {
        ...
        // 2. Player feet must be above the enemy's vertical center (more
        // generous than top edge)
        ...
        bool feetAboveCenter =
            playerBottom <
            enemyCenterY + 5.0f; // Allow slightly below center if falling fast

        float enemyCenterX = enemyPos.x;
        float playerCenterX = pPos.x;
        float horizontalDistance = std::abs(playerCenterX - enemyCenterX);
        float maxHorizontalDistance =
            (enemyBounds.size.x / 2.0f) + 6.0f; // Generous horizontal overlap

        bool horizontallyAligned = horizontalDistance < maxHorizontalDistance;

        // If falling and reasonably above, count as stomp even if overlapping
        // slightly
